...
# REF: https://medium.com/google-cloud/counting-gemini-text-tokens-locally-with-the-vertex-ai-sdk-78979fea6244
...
class GeminiDatasetGenerator(ABC):
    tokenizer = tokenization.get_tokenizer_for_model(settings.OPENAI_MODEL_ID)
...
"""
    ...
    def get_prompt(cls, document: CleanedDocument) -> GenerateDatasetSamplesPrompt:
        ...
        prompt_tokens = cls.tokenizer.count_tokens(prompt)
        prompt = GenerateDatasetSamplesPrompt(
            template=prompt_template.template,
            input_variables=input_variables,
            content=prompt,
            num_tokens=prompt_tokens.total_tokens,
    ...
    def generate(
        ...
        for category, category_prompts in prompts.items():
            ...
            batches = utils.misc.batch(langchain_category_prompts, size=1)
...
class InstructionDatasetGenerator(GeminiDatasetGenerator):
...
class PreferenceDatasetGenerator(GeminiDatasetGenerator):
...
def get_dataset_generator(dataset_type: DatasetType) -> type[GeminiDatasetGenerator]:
